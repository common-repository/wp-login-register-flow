...
class Wp_Login_Register_Flow_Public {
	...
	public function enqueue_scripts() {
		...
		# define ajax objext
		wp_localize_script( $this->plugin_name,'wplrf_ajax_object',array('ajaxurl' => admin_url( 'admin-ajax.php' ),'wplrf_ajax_nonce' => wp_create_nonce('wplrf_ajax_nonce')));

	}

	/**
	 # Add wplrf popup html in footer
	 #
	 # @since    1.0.0
	 */
	public function wplrf_html(){
		//ob_start();
		# include popup in footer
	...
	/**
	 ...
	 # @since    1.0.0
	 */
	public function wplrf_hide_admin_bar(){
		$hide_admin_bar = esc_attr(get_option('wplrf-show-admin-bar'));
		$hide_admin_bar = !empty($hide_admin_bar) ? $hide_admin_bar : "no";
		 if(!empty($hide_admin_bar) && $hide_admin_bar == "yes"){
